...
public class PebbleTemplatingConfiguration {
  ...
  @Bean
  public Map<String, PebbleTemplate> getTemplates() {
    Map<String, PebbleTemplate> templateMap = Stream.of(externalTemplatesLocation, classpathTemplatesLocation)
        .map(Paths::get)
        .filter(Files::exists)
    ...
    if (templateMap.isEmpty()) {
      LOG.debug("No HTML templates found");
    }
  ...
  private Function<Path, String> getTemplateName() {
    return location -> location.getFileName().toString();
  }
