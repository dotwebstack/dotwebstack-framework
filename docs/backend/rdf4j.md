...
property is used to point to the prefix of the shapesfile. This prefix is ignored when mapping GraphQL types, generated
by resolving the `dotwebstack.yaml`, to the node shapes in the `shapes` file.
...
type, generated by resolving the `dotwebstack.yaml`, is matched on a `node shape` in the `shapes` file after stripping
the prefix from the name of the node shape. This means that the `Brewery` from the `dotwebstack.yaml` file is mapped on
the `beer_sh:Brewery` node shape. Fields from `dotwebstack.yaml` file are mapped on property shapes from the 'shapes'
file based on the 'sh:name' property.
