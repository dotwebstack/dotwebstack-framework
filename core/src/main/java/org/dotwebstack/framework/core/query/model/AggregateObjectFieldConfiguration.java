...
public class AggregateObjectFieldConfiguration {
  ...
  public List<AggregateFieldConfiguration> getAggregateFields(boolean stringJoin){
    return aggregateFields.stream()
        .filter(aggregateFieldConfiguration -> stringJoin ? aggregateFieldConfiguration.getAggregateFunctionType()==JOIN : aggregateFieldConfiguration.getAggregateFunctionType() != JOIN)
