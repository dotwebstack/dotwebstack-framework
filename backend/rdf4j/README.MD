...
In the examples in the previous paragraph the filtering was done on the root level of the query. The filters where 
applied to direct fields of a brewery and they only had effect on the breweries that where returned. It is possible 
though to query on and filter indirect properties of the root. 
...
It is also possible to filter a subset of the resultset. If you want the only have the beers with a certain name in 
your resultset, for example. The set of breweries itself is not filtered, but only the set of beers under each brewery:
