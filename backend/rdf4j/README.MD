...
In the example the NodeShape `Dog` uses the `dws:inherits` (dws from \<http://www.dotwebstack.org/\>) the properties of `Pet`. This means that besides the `fetchesBall` property
