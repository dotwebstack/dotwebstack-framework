...
The RDF4J backend supports direct and nested field selections like the following example.
...
([sh:inversePath ex:child] ex:firstname)
```
...
### ZeroOrOne path
The *zero or one* path works in the same way as the 'zero or more' path, but uses the `?` operator and allows paths of
length 0 or 1.
