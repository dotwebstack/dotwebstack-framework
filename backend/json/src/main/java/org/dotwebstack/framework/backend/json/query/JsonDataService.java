...
public class JsonDataService {
  ...
  private JsonNode readJsonDataFile(String fileName) {
    ...
    if (data.isEmpty()) {
      throw invalidConfigurationException(String.format("JSON data file: %s does not exists", JSON_DATA_PATH + fileName));
    }

    try(InputStream jsonDataInputStream = data.get().getInputStream()) {
      ...
      throw illegalStateException(String.format("Encountered IOException while reading %s", JSON_DATA_PATH + fileName));
    }
  }
}
